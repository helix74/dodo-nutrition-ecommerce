...
async function getOrderDetail(id: string): Promise<OrderDetail | null> {
  const order = await writeClient.fetch(
    `*[_type == "order" && (_id == $id || _id == "drafts." + $id)] | order(_updatedAt desc) [0] {
      _id,
      orderNumber,
  ...
  return {
    ...order,
    isDraft: order._id?.startsWith("drafts."),
  };
}
...
export default async function OrderDetailPage({ params }: PageProps) {
  ...
  const statusConfig = getOrderStatus(data.status);
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:items-end">
            ...
            <div className="flex items-center gap-2">
              <RevertButton documentId={id} isDraft={data.isDraft} />
              <PublishButton documentId={id} isDraft={data.isDraft} />
            </div>
          </div>
        </div>
            ...
            <div className="rounded-xl border border-border bg-card p-4 sm:p-6">
              ...
              <div className="mt-4 space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-foreground">{formatPrice(data.total)}</span>
                </div>
          ...
          <div className="space-y-6 lg:col-span-2">
                ...
                <div className="mt-4 space-y-3 text-sm">
                  {data.phone && (
                    <div className="flex items-center justify-between">
                      ...
                      <a href={`tel:${data.phone}`} className="font-medium text-dodo-yellow hover:underline">
                        {data.phone}
                      </a>
                    </div>
                  )}
            ...
            <div className="rounded-xl border border-border bg-card p-4 sm:p-6">
              ...
              <div className="mt-4 space-y-2 text-sm">
                <p className="break-all text-foreground">{data.email}</p>
